...
{
	if (isset($_POST["btnSubmit_delete_polarization"])):
	{
		$sqlSelect =
			"SELECT * FROM polarizations ".
			"WHERE value='".$_POST["select_delete_polarization_value"]."'";
		try
		{
		...
		}
		{
			if(!isset($ex))
				echo
					...
					" has been deleted from table polarizations.";
		}
	}
	else:
	{
?>
				<form action='/administration/#<?php echo basename(__FILE__, '.php'); ?>' method='post'>
					<select name='select_delete_polarization_value'>
<?php
...
?>
					...
					<input type='submit' name='btnSubmit_delete_polarization' value='delete'><br>
				</form>
<?php
	}
	endif;
