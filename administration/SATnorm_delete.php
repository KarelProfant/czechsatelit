...
{
	if (isset($_POST["btnSubmit_delete_SATnorm"])):
	{
		$sqlSelect =
			"SELECT * FROM SATnorms ".
			"WHERE value='".$_POST["select_delete_SATnorm_value"]."'";
		try
		{
		...
		}
		{
			if(!isset($ex))
				echo
					...
					" has been deleted from table SATnorms.";
		}
	}
	else:
	{
?>
				<form action='/administration/#<?php echo basename(__FILE__, '.php'); ?>' method='post'>
					<select name='select_delete_SATnorm_value'>
<?php
...
?>
					...
					<input type='submit' name='btnSubmit_delete_SATnorm' value='delete'><br>
				</form>
<?php
	}
	endif;
